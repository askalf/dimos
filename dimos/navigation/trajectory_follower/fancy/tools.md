...
uv run pytest dimos/navigation/trajectory_follower -q
uv run mypy dimos/navigation/trajectory_follower

# the rust laws; the pyo3 extension is what the parity test imports
uv run maturin develop --uv --release --features python -m dimos/navigation/trajectory_follower/fancy/rust/Cargo.toml
cargo test --release --manifest-path dimos/navigation/trajectory_follower/fancy/rust/Cargo.toml

# the native twin (rust/src/module/follower.rs) runs the same law on the robot
cargo test --features module --manifest-path dimos/navigation/trajectory_follower/fancy/rust/Cargo.toml
