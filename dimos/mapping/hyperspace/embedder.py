...
# The ensemble the size sweep picked (sf_office, 302 keyframes, 4 queries;
# plan.md step 7): two small checkpoints whose per-cell minimum doubles the
# single So400m's cluster precision at 0.8x its recall, at 2.7x (Mac) / 5.9x
# (RTX 5070) its frame rate and 57% of its memory. Adding
...
def parse_member(spec: str) -> tuple[str, int | None, tuple[int, int] | None]:
    """Split a member spec into (checkpoint id, NaFlex budget, tiling).

    ``"google/siglip2-base-patch16-naflex@576"`` -> (id, 576, None) and
    ``"google/siglip2-base-patch16-224#2x3"`` -> (id, None, (2, 3)); a spec
    with neither suffix gives (id, None, None).
    """
    head, _, tiling = spec.partition("#")
    name, _, budget = head.partition("@")
    tiles = None
    if tiling:
        rows, _, cols = tiling.partition("x")
        tiles = (int(rows), int(cols))
        if tiles[0] < 1 or tiles[1] < 1:
            raise ValueError(f"tiling in {spec!r} must be at least 1x1")
    return name.strip(), int(budget) if budget else None, tiles
...
def member_tag(spec: str) -> str:
    ...
    if budget:
        tag = f"{tag}-{budget}"
    if tiles:
        tag = f"{tag}-{tiles[0]}x{tiles[1]}"
    return tag


def tile_image(image: Any, rows: int, cols: int) -> list[Any]:
    """Split a PIL image into ``rows*cols`` non-overlapping crops, row-major.

    Edges are integer splits of the frame, so the crops tile it exactly with
    nothing dropped and nothing counted twice.
    """
    width, height = image.size
    xs = [width * c // cols for c in range(cols + 1)]
    ys = [height * r // rows for r in range(rows + 1)]
    return [
        image.crop((xs[c], ys[r], xs[c + 1], ys[r + 1])) for r in range(rows) for c in range(cols)
    ]


def stitch_tiles(
    grids: list[NDArray[np.float32]], rows: int, cols: int, side: int
) -> NDArray[np.float32]:
    """Lay row-major tile grids (``side x side`` patches each) into one
    ``(rows*side) x (cols*side)`` grid in the frame's own geometry.

    Because the crops do not overlap, every output patch comes from exactly
    one tile: nothing is averaged, so the embeddings stay unit-norm and
    text-aligned.
    """
    dim = grids[0].shape[1]
    out = np.empty((rows * side, cols * side, dim), dtype=np.float32)
    for index, grid in enumerate(grids):
        r, c = divmod(index, cols)
        out[r * side : (r + 1) * side, c * side : (c + 1) * side] = grid.reshape(side, side, dim)
    return np.ascontiguousarray(out.reshape(rows * side * cols * side, dim))
...
class SigLIP2PatchesConfig(SigLIPModelConfig):
    ...
    # Embed the frame as a (rows, cols) grid of crops instead of whole. A
    # fixed-resolution checkpoint squashes the whole frame to a square, so an
    # 848x480 frame goes through a 224x224 model at a 3.8x downscale and a
    # 30 px soda bottle lands inside a single patch. Splitting it 2x3 sends
    # near-square 283x240 crops through the same model, so the same bottle
    # spans patches -- 3x the effective resolution for 6x the forward passes.
    # Crops do not overlap, so the stitched grid is exact: no embedding is
    # ever averaged with another (plan.md: pool scores, never embeddings).
...
class SigLIP2Patches(SigLIPModel):
    ...
    def embed_grids(self, *images: Image) -> list[tuple[NDArray[np.float32], tuple[int, int]]]:
        ...
        if self.config.tiles is not None:
            if self.naflex:
                raise RuntimeError(
                    f"{self.config.model_name} is NaFlex: it already keeps the aspect ratio, "
                    "raise its patch budget instead of tiling"
                )
            return self._embed_tiled(pil_images)
        if not self.naflex:
    ...
    def _embed_tiled(
        self, pil_images: list[Any]
    ) -> list[tuple[NDArray[np.float32], tuple[int, int]]]:
        """Every crop of every frame in one batch, then stitched back per frame."""
        rows, cols = self.config.tiles  # type: ignore[misc]
        crops = [crop for image in pil_images for crop in tile_image(image, rows, cols)]
        with torch.inference_mode():
        ...
        return [
            (
                stitch_tiles(grids[i * per_image : (i + 1) * per_image], rows, cols, side),
                (rows * side, cols * side),
            )
