...
def _decode(answers: Answers):  # type: ignore[no-untyped-def]
    return decode(answers, min_confidence=0.5, stop_threshold=0.7, finished_threshold=0.7)


def test_questions_one_choice_per_axis_plus_stop_and_target() -> None:
    assert set(questions(())) == {"drive.x", "drive.y", "drive.yaw", "stop", "finished"}
...
def test_finished_stops_and_flags() -> None:
    d = _decode(_answers(x="forward", finished=0.9))
    assert d.finished and d.stop and d.is_zero
    assert not _decode(_answers(x="forward", finished=0.5)).finished
