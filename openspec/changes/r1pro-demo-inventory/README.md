...
## September 18, 2026 update: classical open-space demo with tray, on this branch

Everything below this section is the September 17 audit and still applies. Since then, on feat/r1pro-classical-apartment:

- The desktop viewer renders from snapshots in its own process; the tray pick, loaded carry and put-down work in the open-space demo (measured handle spacing, adaptive lift clearance, dock stand-off, fixture-aware footprints, up-front refusal of platforms above the carry height). Seed 5000 headless and desktop runs pass: carton into tray, loaded tray to display_table, re-pick at 0.80 m, delivery to low_bench.
- Grasp ranking runs in a persistent worker process warmed at startup with a 150 s cap; a cross-body reach with a loaded hand repositions the base first. The in-process ranking that froze physics is gone. GraspGenX loads from the local cache.
- Agent load trimmed: compact get_scene, one scene read per request, viewer 10 Hz, unused camera 1 Hz.
- The harness accepts tray_pick::, tray_place::region, and --agent --say --model.

Blockers found on the host dimensional-gpu-0 (Gigabyte AORUS 17X): thermal throttling to 200 to 800 MHz at 90 C with fans at maximum (the simulation runs at 0.1 to 0.3x real time; it ran at 1.0x when cool), a Wi-Fi network advertising dead IPv6 routes (stalls every Python HTTPS call; disable IPv6 or set DIMOS_FORCE_IPV4=1 with the venv-local shim), an invalid OPENAI_API_KEY in the launch environment and an Anthropic key without credits. GPU physics was measured and rejected for this scene: MJX lacks cylinder-box collisions; MuJoCo Warp needs noslip off and then the tray and props fall through the table, with a 1.48 ms graph-captured step and 54 ms state readback.

Apartment scene: tray delivery reaches only the worktable (dining table too cluttered for this tray, kitchen counter too high). The elbows-in carry posture was tried and reverted (elbow flip dropped the carton); a compact carry needs a posture-continuity constraint. Whole-body IK over base, torso and arm in one Pink QP is the recommended follow-up.
