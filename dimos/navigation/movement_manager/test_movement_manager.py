...
def test_manipulation_outranks_nav(manager_and_captured):
    """A whole-body plan's base segment must not be steered by the route follower."""
    manager, captured = manager_and_captured
    manager.config.manip_cooldown_sec = 10.0

    manager._on_manip(_twist(lx=0.2))
    assert len(captured.cmd_vel) == 1
    assert captured.cmd_vel[-1].linear.x == 0.2

    manager._on_nav(_twist(lx=0.9))
    assert len(captured.cmd_vel) == 1, "nav leaked through while a plan held the base"


def test_teleop_outranks_manipulation(manager_and_captured):
    """The operator still wins: teleop overrides a plan driving the base."""
    manager, captured = manager_and_captured
    manager.config.tele_cooldown_sec = 10.0

    manager._on_teleop(_twist(lx=0.3))
    before = len(captured.cmd_vel)
    manager._on_manip(_twist(lx=0.2))
    assert len(captured.cmd_vel) == before, "a plan drove the base during teleop"


def test_nav_resumes_after_the_manipulation_cooldown(manager_and_captured):
    """The gaps between a plan's segments are covered, but a finished plan hands back."""
    manager, captured = manager_and_captured
    manager.config.manip_cooldown_sec = 0.05

    manager._on_manip(_twist(lx=0.2))
    before = len(captured.cmd_vel)
    time.sleep(0.1)
    manager._on_nav(_twist(lx=0.9))
    assert len(captured.cmd_vel) == before + 1
    assert captured.cmd_vel[-1].linear.x == 0.9


def test_manipulation_resumes_after_the_teleop_cooldown(manager_and_captured):
    manager, captured = manager_and_captured
    manager.config.tele_cooldown_sec = 0.05
