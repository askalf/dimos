...
@pytest.mark.parametrize("source", ["rpc", "buttons", "policy"])
def test_all_stop_sources_use_same_latched_damping(connection, source):
    publisher = _wire(connection, soft_start_seconds=1000.0)
    connection._on_motor_command(_command())
    buttons = Buttons()
    buttons.left_primary = buttons.left_secondary = True
    buttons.right_primary = buttons.right_secondary = True
    operations = {
        "rpc": lambda: connection.set_estop(True),
        "buttons": lambda: connection._on_teleop_buttons(buttons),
