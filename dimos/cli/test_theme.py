...
def cards() -> dict[str, list[str]]:
    return {
        ...
        "logged_out": cloud._logged_out_card(),
    }
...
@pytest.mark.parametrize("cols", [40, 60, 80, 88, 89, 100, 120])
def test_reveal_only_animates_when_the_wordmark_fits(
    cols: int, tty: None, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    """Told to ignore terminal dimensions, the effect would overflow rather than
    crop — so anything narrower than the wordmark must skip it, not wrap it."""
    monkeypatch.setattr(theme, "term_width", lambda: cols)
    rows = cloud._wordmark()
    needed = max(theme.visible_len(r) for r in rows)
    cloud._reveal(rows)
