...
def in_process_command(
    ...
    """This package's own indexer, for a recording siglipify cannot read.

    siglipify knows only `Image` streams, and `lite_record` writes its colour as a
    `CompressedImage` -- the demo recording included. This module reads that perfectly
    well, because `open_recording` decodes it on the way out, so the embeddings it would
    have to give up on are one subprocess away. Slower than siglipify, and the only thing
    that works there.
    """
    return [
        sys.executable,
        "-m",
        "dimos.teleop.memory_world.visual_search",
        store_path,
        "--image-stream",
        image_stream,
        "--model",
        ...
        "--stride",
        str(stride),
        # The names the MODULE resolved, not the subprocess's defaults. It defaulted to a
        # tf stream called `tf`, which the module's own detection had already found under
        # another name on some recordings -- and the subprocess then died with a KeyError
        # rather than indexing anything.
        "--tf-stream",
        tf_stream,
        "--world-frame",
        world_frame,
        # ... and EVERY other name the module resolved, for the same reason. Measured, one
        # at a time, on a recording the module indexes perfectly: with the camera frame
        # left out, the subprocess fell back to the images' own `frame_id` -- empty on that
        # rig, which is what `camera_optical_frame` is set for -- every lookup returned
        # None and it reported "added 0 frames" and exited 0, so the job said done. With
        # the index stream left out it wrote its embeddings into the name it makes up from
        # the model, and the module read `count: 0` from the stream it was configured with.
        # With the tolerance left out it used its own 0.1 s against tf stamped 0.5 s from
        # the images and dropped the frames the module places.
        "--tf-tolerance",
        str(tf_tolerance_s),
        *(("--camera-frame", camera_frame) if camera_frame else ()),
        *(("--index-stream", index_stream) if index_stream else ()),
    ]
