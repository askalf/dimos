...
Preserved evidence: `recordings/r1pro-classical-navigation-fix/seed-1646757217/result.json`, all per-action JSON/snapshots and the compiled model beside it. The recordings directory is shared with the ACT worktree; this adds evidence without changing ACT code/checkpoints. Large immutable temporary model copies were deduplicated with hard links, preserving their bytes and paths. `validated-navigation-sources.json` hashes all relevant navigation sources; they match the final tested code. Original supervisor status: `/tmp/r1pro-nav-return/stack-v6-status.json`. Supervisor and child have exited; no automated test or training remains running.

Maximum combined translation/yaw tracking deviation was **1.85 cm** on dining transit, **0.49 cm** on kitchen transit and **1.67 cm** on the formerly failing return. Transit reserves 6 cm and stops at 4 cm deviation; arrival planning reserves another 2 cm for settling. These measurements cover this sequence, not an arbitrary-layout success rate.

The original failure is session `recordings/r1pro-primitives/86b14d4cc63f46ab83dc671a63af545a`, action 006. `wheel_motor_link3` contacted the display cabinet (`dc4b6a1b56faf0b0ac679141c9c53f1449bf984a-articulated-001`). Saved-state physics replay reproduced contact at (-0.2601, -1.3129), close to the user's (-0.2568, -1.3126). Geometric shortening had converted the native route into sharp corners; measured cross-track error reached 15.6 cm against a 2 cm allowance.

The fix preserves native waypoints, checks the complete robot/cargo sweep with configurable clearance, searches small local offsets only around obstructed sections, monitors combined translation/yaw deviation, and saves commanded routes before execution. Arrival selection checks the shortest turn and a nearby clear stop. The classical simulator override now uses the shared approach checker; changing only its parent was insufficient. Live contact thresholds and physical object geometry are unchanged.

Validation: **32 route/transport tests** and **21 execution/navigation tests** passed, strict mypy passed the changed implementations, and commit hooks passed. Standalone controller/physics replays reproduced the old collision, passed the corrected routes, and stopped the old route before contact with the new tracking guard. The final full-stack run uses local MCP; the user separately exercised HumanCLI successfully. No external language-provider test was sent.

One intermediate run was interrupted when the workstation filesystem filled. Approximately 8 GB of duplicate temporary compiled models was reclaimed without deleting recordings or training data. The final detached supervisor imposed a 512 MB output limit, a 32 MB log limit and a stop below 1 GB free; it completed with exit code 0. User processes were not stopped.
...
- **Background work:** all our delivery, promotion and navigation validation supervisors completed. No automated test or training is running. Other user processes were not stopped by this work.
