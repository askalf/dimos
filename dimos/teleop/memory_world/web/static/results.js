// Stepping through an answer's places: next/prev flies the camera to each one,
// lights it up, shows only its evidence frames, and can ask the server for a
// route to it.
...
export class ResultsNav {
    constructor({ scene, flight, baseUrl, diag, ui }) {
        this.scene = scene;
        this.flight = flight;
    ...
    async navigate() {
        ...
        try {
            ...
            // Where the person who pressed this is standing. The server keeps ONE
            // `_viewer_position`, written by whichever client sent a pose last, so with
            // two people in the same world the route was drawn from the other one's feet
            // and reported as fact. The viewer that asks is the viewer that knows.
            const start = this.scene && this.scene.getViewerRobotPosition
                ? this.scene.getViewerRobotPosition()
                : null;
            const response = await fetch(`${this.baseUrl}/navigate`, {
                ...
                body: JSON.stringify({
                    ...
                    ...(view >= 0 ? { view } : {}),
                    ...(start && start.every(Number.isFinite) ? { start } : {}),
                }),
            });
